...
from priv_eval.mia_classifier import LABEL_IN, LABEL_OUT, generate_mia_shadow_data_shufflesplit
...
def evaluate_mia(GenModel, attacksList, rawWithoutTargets, targetRecords, targetIDs, rawAidx, rawTindices, sizeRawT, sizeSynT, nSynT, nSynA, nShadows, metadata):
    ...
    # tasks = [(
    #          GenModel, attacksList, targetRecords.loc[[tid], :], tid, rawWithoutTargets, metadata, rawAidx, rawTindices,
    #          sizeRawT, sizeSynT, nSynT, nSynA, nShadows) for tid in targetIDs]
    # resultsList = map(worker_run_mia, tasks)
    ...
    for AM in attacksList:
        logger.info(
            f'Mean record privacy gain across {len(targetRecords)} Targets with Attack {AM.__name__}: {mean(results[AM.__name__]["RecordPrivacyGain"])}%')
...
def worker_run_mia(params):
    ...
    synA, labelsSynA = generate_mia_shadow_data_shufflesplit(GenModel, target, rawA, sizeRawT, sizeSynT, nShadows,
                                                             nSynA)
    ...
    for nr, rt in enumerate(rawTindices):
        ...
        # TODO: This is inefficient and more noisy as reference model trained without target could be the same for all targets
        if GenModel.datatype is DataFrame:
...
def run_mia(Attack, synT, labelsSynT, targetID, nr):
    ...
    results = {
        'TargetID': [targetID for _ in range(len(labelsSynT))],
        'TestRun': [f'Run {nr + 1}' for _ in range(len(labelsSynT))],
...
def evaluate_ai(GenModel, rawWithoutTargets, targetRecords, targetIDs, rawA, rawTindices, sensitiveAttribute, sizeSynT, nSynT, metadata):
    ...
    for nr, rt in enumerate(rawTindices):
        ...
        with Pool(processes=PROCESSES) as pool:
            ...
            resList = map(worker_run_mleai, tasks)
