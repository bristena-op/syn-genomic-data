...
from priv_eval.feature_sets.feature_set import FeatureSet
from priv_eval.datagen import CATEGORICAL, CONTINUOUS, ORDINAL
...
class HistogramFeatureSet(FeatureSet):
    def __init__(self, datatype, metadata, nbins=10):
        ...
        for cidx in self.cat_attr_idx:
            ...
            self.cat_attr_names.append(int(col['name']))
            self.category_codes[int(col['name'])] = col['i2s']
        ...
        for cidx in self.num_attr_idx:
            col = metadata['columns'][cidx]
            self.num_attr_names.append(int(col['name']))
            self.histogram_bins[int(col['name'])] = linspace(col['min'], col['max'], nbins+1)
