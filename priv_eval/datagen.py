...
def load_local_data_as_df(filename, transpose = True):
    df = pd.read_csv(f'{filename}.hap', sep= '\t', header=None)
    dat = df.to_numpy()
    if transpose == True:
        dat = np.transpose(dat)
    df = pd.DataFrame(data=dat[1:, :], columns=dat[0,:])
...
def load_local_data_as_array(filename):
    df = pd.read_csv(f'{filename}.csv', sep= '\t', header=None)
    with open(f'{filename}.json') as f:
        metadata = json.load(f)
    metadata['categorical_columns'], metadata['ordinal_columns'], metadata['continuous_columns'] = _get_columns(metadata)
