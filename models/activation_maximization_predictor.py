...
parser.add_argument('--data_loc', type=str, default='/home/nkilloran/datasets/leo_binding_data/Max/sim_ground_truth',
                    help='Data location')
parser.add_argument('--data_start', type=int, default=0,
                    help='Line number to start when parsing data (useful for ignoring header)')
...
inputs = tf.Variable(tf.constant(0., shape=[args.batch_size, args.max_seq_len, vocab_size]),
...
filters = tf.Variable(tf.truncated_normal(stddev=0.01, shape=[args.filter_size, len(charmap),
                                                              args.num_filters]))  # (width, in_channels, out_channels)
...
max_pool = tf.nn.max_pool(features, ksize=[1, 1, args.max_seq_len, 1], strides=[1, 1, args.max_seq_len, 1],
                          padding="SAME")
avg_pool = tf.nn.avg_pool(features, ksize=[1, 1, args.max_seq_len, 1], strides=[1, 1, args.max_seq_len, 1],
                          padding="SAME")
...
W_final = tf.Variable(
    tf.truncated_normal(shape=[args.hidden_size, 1], stddev=tf.sqrt(2. / tf.cast(args.hidden_size, tf.float32))))
...
for idx in range(args.num_epochs):
    ...
    # save checkpoint
    checkpoint_epoch = args.checkpoint_iters and (epoch_count % args.checkpoint_iters == 0) or (
    idx == args.num_epochs - 1)
    if checkpoint_epoch:
