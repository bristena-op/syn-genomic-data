#https://github.com/co9olguy/Generating-and-designing-DNA/blob/master/scripts/plug_and_play.py
import os
import argparse
...
parser.add_argument('--target', default="max",
                    help="Optimization target. Can be either 'max', 'min', or a target score number given as a float")
...
if args.checkpoint:
    ...
    batch_size, latent_dim = session.run(tf.shape(latents))
    latent_vars = [c for c in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES) if 'generator/latent_vars' in c.name][
        0]
    sequence_vars = \
    [c for c in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES) if 'predictor/Input_layer' in c.name][0]
    ...
    if args.optimizer == "adam":
        ...
        adam_initializers = [var.initializer for var in tf.global_variables() if
                             'Adam' in var.name or 'beta' in var.name]
...
for ctr in range(args.iterations):
    ...
    if true_ctr == checkpoint_baseline + 1 or true_ctr % args.log_interval == 0:
        ...
        print(
            "Iter {}: {}: score: {:.6f}; mean score: {:.6f}; std: {:.6f}; mean score (one-hot): {:.6f}".format(true_ctr,
                                                                                                               seq0,
                                                                                                               preds[0],
                                                                                                               mean_pred,
                                                                                                               np.std(
        ...
        plt.plot(np.linspace(checkpoint_baseline, true_ctr, len(means)), means, color='C2',
                 label='Mean score of generated sequences');
        if args.plot_mode == "fill":
            plt.fill_between(np.linspace(checkpoint_baseline, true_ctr, len(means)), mins, maxes, color='C0', alpha=0.5,
        ...
        plt.plot(np.linspace(checkpoint_baseline, true_ctr, len(means_onehot)), means_onehot, color='C1', ls='--',
