...
def loadRecombData(haplotypeFileName, recombFileName):
    ...
    dat = pd.read_csv(haplotypeFileName, sep='\t', header=None)
    import pdb; pdb.set_trace()
    ...
    dat = pd.read_csv(recombFileName, sep='\t', header=None)
...
def genRandSeq(kwargs):
    ...
    if kwargs['modelName'] == 'PubLDModel':
        ...
        model = RecombModel(SNPRefList, list(map(lambda u: u[:200], haplotype)), geneticDist)
    ...
    for i in range(int(kwargs['sampleSize'])):
        ...
        h0, h1 = to_haplo(seq)
        # import pdb; pdb.set_trace()
        outFile.write(' '.join(map(str, h0)) + '\n')
        outFile.write(' '.join(map(str, h1)) + '\n')
    outFile.close()
def to_haplo(seq):
    h0 ,h1=[], []
    for item in seq:
...
if __name__ == "__main__":
    ...
    #                 'sampleSize':100})
    # genRandSeq({'genotypeFileName': "../../datasets/chr13/small_genotypes_chr13_CHB.txt",
    #             'modelName': 'RecombModel',
    #             'haplotypeFileName': '../../datasets/chr13/small_CHB.chr13.hap',
    #             'geneticDistFileName': '../../datasets/chr13/small_genetic_map_chr13_combined_b36.txt',
    #             'outFileName': '../../syn_data/recomb_random_chr13_CHB.csv',
    #             'sampleSize': 150})

    genRandSeq({'genotypeFileName': "../../datasets/chr13/small_genotypes_chr13_CEU.txt",
                'modelName': 'RecombModel',
                'haplotypeFileName': '../../datasets/chr13/small_CEU.chr13.hap',
                'geneticDistFileName': '../../datasets/chr13/small_genetic_map_chr13_combined_b36.txt',
                'outFileName': '../../syn_data/recomb_hap_chr13_CEU.csv',
                'sampleSize': 200})